...
`--workers N` (`--ld-workers`; use `1` with `--profile`), `--assume-variant1-indexed`,
`--region START-END`, `--pheno`, `--rebuild-subset`. Outputs and the cached slice land in
`benchmarks/_out/` (gitignored).
...
**Recommendation:** enable `--ld-assume-variant1-indexed` for the finngen_r12 LD panel — it
is the dominant win and verified correct on real data. The other optimizations (asTuple
parsing, header-index hoisting, greedy de-quadratic) help the parse/grouping stages and the
wide path when narrow fetch isn't applicable.
