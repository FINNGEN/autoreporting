...
from scipy.special import ndtri_exp # type: ignore
...
def _chi2_isf_df1(pval:float, pval_is_mlog10p:bool)->float:
    """chi2.isf(p, df=1) computed entirely in log space so ultra-significant
    p-values don't underflow. Uses chi2_1.isf(p) == ndtri(p/2)**2, and
    ndtri_exp(log(p/2)) to get ndtri(p/2) without materializing p.
    """
    with np.errstate(divide="ignore"):  # p==0 -> log(p)==-inf is intended, flows to a 0 threshold
        logp = (-pval*np.log(10)) if pval_is_mlog10p else np.log(pval)
    z = ndtri_exp(logp - np.log(2))
    return float(z*z)
...
def ld_threshold(ld_thresh:float, mode: LDMode,pval:float, pval_is_mlog10p:bool=False)->float:
    if mode.value == LDMode.DYNAMIC.value:
        return float(min(ld_thresh/_chi2_isf_df1(pval, pval_is_mlog10p),1.0))
    else:
        return ld_thresh

def _passes_threshold_eq(value:float, threshold:float, pval_is_mlog10p:bool)->bool:
    """Inclusive significance check.
    For raw pval smaller is more significant (value <= threshold); for mlog10p larger is (value >= threshold).
    """
...
def _sort_most_significant_last(items, key, pval_is_mlog10p:bool):
    """Sort so the most significant variant ends up last (for stack popping).
    raw pval: descending (smallest pval last); mlog10p: ascending (largest mlog10p last).
    """
...
def simple_grouping(summstat_resource:TabixResource, options:GroupingOptions) -> List[PeakLocus]:
    ...
    for l in summstat_resource.fileobject.fetch():
        ...
        try:
            ...
            continue
        if _passes_threshold_eq(pval, options.p2_threshold, options.pval_is_mlog10p):
            var = Var(Variant(
                cols[hdi[cpra[0]]],
                int(cols[hdi[cpra[1]]]),
                cols[hdi[cpra[2]]],
            ...
            if _passes_threshold_eq(pval, options.p1_threshold, options.pval_is_mlog10p):
