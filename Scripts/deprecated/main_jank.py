...
import pandas as pd 
import numpy as np
import gws_fetch, compare, autoreporting_utils,top_report
...
from grouping_model import CSInfo, Grouping, LDMode, PhenoInfo, PhenoData, SummstatColumns,GroupingOptions, Locus, PeakLocus, CSLocus, Var
...
def main(args):
    ...
    #jank layer
    #depending on the type of grouping, create loci.
    fetch_df.to_csv("quickstart_df.tsv",sep="\t")
    ...
    extra_cols_annotation = ExtraColAnnotation(TabixOptions(args.gws_fpath,
        column_names.c,
        column_names.p,
        column_names.r,
        column_names.a,
    ...
    #resume with sane pandasless code
    phenotype_info = None
    print("Making phenotype info")
    ...
    print("forming phenotype")
    phenodata = PhenoData(phenotype_info,list_of_loci)
    ### annotate
    print("annotating phenodata")
    ...
    top_report_options = TopReportOptions(args.strict_group_r2,
        ...
        args.efo_traits,
        args.extra_cols,
        ldModeFactory(args.dynamic_r2_chisq),
        r2_thresh)
    ...
    with open(args.report_out,"w") as report_file, open(args.top_report_out,"w") as top_file:
        ...
        debug_out = generate_top_report(phenodata,top_file,top_report_options)
    #print(debug_out)
